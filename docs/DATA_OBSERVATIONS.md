...
\### OBS-018 — Invalid image URLs in bulk export

\*\*Date:\*\* 22 May 2026

\*\*Finding:\*\* Products without front-of-pack images have image\_url containing

"/invalid/" — e.g. ".../images/products/invalid/front\_en.6.400.jpg"

This is OFF's deliberate placeholder, not a random 404.



\*\*Filter to apply before v3 vision processing:\*\*

df = df\[\~df\["image\_url"].str.contains("/invalid/", na=True)]



\*\*Where to apply:\*\*

1\. bulk\_load.py — exclude during initial category filter

2\. v3 smart sampling script — second filter before Azure API calls



\*\*Expected valid image rate:\*\* \~60-70% of full database

(well-documented products almost always have valid images;

incomplete entries like barcode-only records do not)
